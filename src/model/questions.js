...
async function getMaxAttempt(questionId) {
  try {
    const maxAttempt = query(
      dbCollection,
      where("question_id", "==", questionId),
    );
    const querySnapshot = await getDocs(maxAttempt);
    if (!querySnapshot.docs[0].data()) {
      return 0;
    }
    return querySnapshot.docs[0]?.data()?.max_attempt_time??0;
  } catch (error) {
    console.error(`getAllQuestionByAssignment: ${error}`);
  }
}
...
export { createNewQuestion, getAllQuestionByAssignment, updateQuestion, getMaxAttempt };
