...
const SubmittedQuestions = () => {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      const sdata = await getAllActiveAssignmnetByStudent(
        assignment_id,
        user.uid,
      );
        ...
        <div className="card shadow mb-4" key={index}>
          ...
          {open === index && (
            <div className="card-body">
              <p>
                <strong>Your Answer:</strong> {que.answer}
              </p>
              <hr />
              <p className="d-flex align-items-center justify-content-between flex-wrap mb-0">
                ...
                <span className="text-muted small">
                  Marks: <strong>{que.mark}</strong>, Attempts:
                  <strong>
                    {" "}
                    {que.max_number_of_attempts}/{que.max_attempts}
                  </strong>
                </span>
              </p>
            </div>
